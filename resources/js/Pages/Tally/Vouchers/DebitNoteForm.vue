...
<template>
    <VoucherGuide :show="showGuide" voucher-type="DebitNote" @close="showGuide = false" />
    ...
    <div v-if="mode !== 'accounting'" class="border border-gray-200 rounded-xl overflow-hidden">
        ...
        <div v-if="form.inventory_entries.length" class="border-t border-violet-100 bg-violet-50 px-4 py-3 flex items-center gap-3">
            ...
            <input v-model="commonSalesLedger" type="text" list="dn-purchase-ledger-list"
            ...
            <datalist id="dn-purchase-ledger-list">
                <option v-for="l in purchaseLedgers" :key="l.id" :value="l.ledger_name" />
            </datalist>
            <span class="text-xs text-violet-500 whitespace-nowrap">Applies to all items · <span class="font-medium">Purchase Accounts</span> group</span>
        </div>
    </div>
    ...
    <div class="border border-gray-200 rounded-xl overflow-hidden">
        ...
        <p v-else class="px-4 py-3 text-xs text-gray-400 italic">
            No ledger lines yet.
            <template v-if="mode === 'item'">Add the supplier ledger (Cr + Party) and tax/charge ledgers (Dr).</template>
            <template v-else-if="mode === 'accounting'">Add the party ledger (Cr + Party) and purchase/expense ledger (Dr).</template>
