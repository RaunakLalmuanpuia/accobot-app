...
class MobileAdminBillingController extends Controller
{
    ...
    public function changePlan(Request $request, Tenant $tenant)
    {
        ...
        AuditEvent::log(
            ...
            ['from' => $old, 'to' => $plan->slug, 'source' => 'admin', 'channel' => 'mobile'],
            auth()->id(),
            $tenant->id,
        );
    ...
    public function changePlanAndRebill(Request $request, Tenant $tenant, RazorpayService $razorpay)
    {
        ...
        DB::transaction(function () use ($sub, $plan, $result, $old, $tenant) {
            ...
            AuditEvent::log(
                ...
                ['from' => $old, 'to' => $plan->slug, 'razorpay_sub' => $result['subscription_id'], 'source' => 'admin', 'channel' => 'mobile'],
                auth()->id(),
                $tenant->id,
            );
        });
    ...
    public function cancel(Tenant $tenant, RazorpayService $razorpay)
    {
        ...
        AuditEvent::log(
            ...
            ['plan' => $sub->plan?->slug, 'source' => 'admin', 'channel' => 'mobile'],
            auth()->id(),
            $tenant->id,
        );
    ...
    public function grantTrial(Request $request, Tenant $tenant)
    {
        ...
        AuditEvent::log(
            ...
            ['trial_ends_at' => $request->trial_ends_at, 'source' => 'admin', 'channel' => 'mobile'],
            auth()->id(),
            $tenant->id,
        );
    ...
    public function overrideStatus(Request $request, Tenant $tenant)
    {
        ...
        AuditEvent::log(
            ...
            ['status' => $request->status, 'plan_id' => $request->plan_id, 'source' => 'admin', 'channel' => 'mobile'],
            auth()->id(),
            $tenant->id,
        );
