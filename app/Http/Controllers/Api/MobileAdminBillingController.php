...
class MobileAdminBillingController extends Controller
{
    public function index()
    {
        ...
        return response()->json(compact('stats', 'rows', 'plans'));
    }
    ...
    public function changePlan(Request $request, Tenant $tenant)
    {
        ...
        AuditEvent::log(
            ...
            ['from' => $old, 'to' => $plan->slug, 'source' => 'admin'],
            auth()->id(),
            $tenant->id,
        );
    ...
    public function changePlanAndRebill(Request $request, Tenant $tenant, RazorpayService $razorpay)
    {
        ...
        AuditEvent::log(
            ...
            ['from' => $old, 'to' => $plan->slug, 'razorpay_sub' => $result['subscription_id'], 'source' => 'admin'],
            auth()->id(),
            $tenant->id,
        );

        return response()->json([
            'message'     => "Plan changed to {$plan->name}. Share the payment link with the tenant.",
    ...
    public function cancel(Tenant $tenant, RazorpayService $razorpay)
    {
        ...
        AuditEvent::log(
            ...
            ['plan' => $sub->plan?->slug, 'source' => 'admin'],
            auth()->id(),
            $tenant->id,
        );
    ...
    public function grantTrial(Request $request, Tenant $tenant)
    {
        ...
        Subscription::updateOrCreate(
            ['tenant_id' => $tenant->id],
            [
                'plan_id'              => $plan?->id ?? $tenant->subscription?->plan_id,
                'status'               => 'trialing',
        ...
        AuditEvent::log(
            ...
            ['trial_ends_at' => $request->trial_ends_at, 'source' => 'admin'],
            auth()->id(),
            $tenant->id,
        );

        return response()->json(['message' => "Trial granted until {$request->trial_ends_at}."]);
    }
    ...
    public function overrideStatus(Request $request, Tenant $tenant)
    {
        ...
        if ($request->filled('plan_id'))       $updates['plan_id']       = $request->plan_id;
        if ($request->filled('trial_ends_at')) $updates['trial_ends_at'] = $request->trial_ends_at;
        if ($request->status !== 'cancelled')  $updates['cancelled_at']  = null;

        $sub->update($updates);

        AuditEvent::log(
            'subscription.status_overridden',
            ['status' => $request->status, 'plan_id' => $request->plan_id, 'source' => 'admin'],
            auth()->id(),
            $tenant->id,
        );

        return response()->json(['message' => "Status set to {$request->status}."]);
    }
}
