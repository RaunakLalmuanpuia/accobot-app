...
| SalesVoucher.vue | (component, rendered inside Vouchers.vue slide-over) | Tally Prime–style Sales invoice form: Invoice# + Date header; Party Name (datalist filtered to Sundry Debtors) + Billing Address; Item Invoice / Accounting Invoice toggle (UI-only `salesMode` ref); Item mode shows a compact table (Item, UOM, Billed Qty, Rate, Disc%, Amount) with a ▼ expand per row revealing Actual Qty, Sales Ledger, HSN, IGST%, CESS%, Tax Amount, Godown, Batch, MRP, Item Code, Group Name, Deemed Positive, plus collapsible BatchAllocations and AccountingAllocations; Ledger Entries section with Bill References per entry; Summary panel (Taxable Value computed from items + editable Grand Total with ↺ reset); Narration, Place of Supply, Is Invoice; collapsible Buyer Details, Consignee Details, e-Invoice sections. `expandedItems` is a reactive Set that tracks which item rows are expanded; indices are shifted on removal. No new backend fields — uses existing `inventory_entries` and `ledger_entries`. `salesMode` is UI-only and not sent to backend. |
