...
| GET | /t/{tenant}/tally/stock-groups | tally.stock-groups.index | integrations.view |
...
| GET | /t/{tenant}/tally/stock-categories | tally.stock-categories.index | integrations.view |
...
| GET | /t/{tenant}/tally/godowns | tally.godowns.index | integrations.view |
...
| StockMasters/Index.vue | tally.stock-masters.index | Overview — 4 count cards (Stock Groups, Stock Categories, Godowns, Units) each linking to the corresponding sub-page. |
| StockMasters/StockGroups.vue | tally.stock-groups.index | Stock Groups — CRUD slide-over (max-w-lg, Tally Prime tally-row layout). Fields: Name*, Parent Group (select, defaults to "Primary"), Costing Method (select), Valuation Method (select), Batch-wise / Perishable / Add Quantities checkboxes, Aliases. Delete distinguishes synced vs unsynced: synced marks inactive + queues Tally deletion; unsynced hard-deletes. |
| StockMasters/StockCategories.vue | tally.stock-categories.index | Stock Categories — CRUD slide-over. Fields: Name*, Parent Category (select, defaults to "Primary"), Aliases. Same delete behaviour as above. |
| StockMasters/Godowns.vue | tally.godowns.index | Godowns — CRUD slide-over. Fields: Name*, Under (select), Has No Space / Has No Stock / Is External / Is Internal checkboxes. |
| StockMasters/Units.vue | tally.units.index | Units of Measure (max-w-lg, Tally Prime tally-row layout). Fields: Name/Symbol* (always the same), Formal Name, Decimal Places, UQC. Symbol auto-set to Name in backend. DB unique constraint on (tenant_id, name). |
...
| `TallyDataController` | Data browse: ledgerGroups(), ledgers(), stockItems(), vouchers(), voucherShow(), statutoryMasters(), payroll(). Stock master sub-pages: stockMasters() (index/counts), stockGroups(), stockCategories(), godowns(), units(). Passes `ledgerGroups` (id/name/under_name objects) for the ledger group dropdown; `stockGroupNames`, `stockCategoryNames` for other master dropdowns; `ledgers` (id/ledger_name/group_name objects), `stockItems` (id/name/hsn_code/unit_name/igst_rate/cess_rate/stock_group_name/mrp_rate objects), and `godowns` (id/name objects) for voucher form auto-fill and dropdowns. |
