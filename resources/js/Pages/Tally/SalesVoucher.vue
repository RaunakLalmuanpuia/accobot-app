...
const sundryDebtorLedgers = computed(() => {
    const filtered = props.ledgers.filter(l =>
        l.group_name?.toLowerCase().includes('sundry debtor')
    )
...
<template>
    ...
    <div class="grid grid-cols-2 gap-3">
        <div>
            ...
            <datalist id="sales-party-list">
                <option v-for="l in sundryDebtorLedgers" :key="l.id" :value="l.ledger_name" />
            </datalist>
        </div>
        <div>
    ...
    <div class="flex items-center gap-1 p-1 bg-gray-100 rounded-lg w-fit">
        ...
        <button type="button" @click="salesMode = 'accounting'"
                :class="['px-4 py-1.5 text-sm font-medium rounded-md transition',
                         salesMode === 'accounting'
                             ? 'bg-white text-violet-700 shadow-sm'
                             : 'text-gray-500 hover:text-gray-700']">
            Accounting Invoice
        </button>
            ...
            <div v-if="expandedItems.has(i)"
                ...
                <div class="grid grid-cols-3 gap-2">
                    ...
                    <div>
                        ...
                        <input v-model="ie.sales_ledger" type="text" list="sales-ledger-list" placeholder="Type or select…"
                               class="w-full rounded border border-gray-300 px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-violet-500" />
                    </div>
                    <div>
    ...
    <div>
        <div class="flex items-center justify-between mb-2">
            <h3 class="text-sm font-semibold text-gray-700">
                {{ salesMode === 'item' ? 'Ledger Entries' : 'Accounting Lines' }}
            </h3>
    ...
    <div class="rounded-lg bg-violet-50 border border-violet-200 p-4 space-y-2">
        ...
        <div class="flex items-center justify-between text-sm pt-1 border-t border-violet-200">
            ...
            <div class="flex items-center gap-2">
                <input v-model="form.voucher_total"
                       ...
                       class="w-36 text-right rounded border border-violet-300 px-2 py-1 text-sm font-mono font-medium focus:outline-none focus:ring-2 focus:ring-violet-500 bg-white" />
        ...
        <div class="px-4 pb-4 pt-3 space-y-3">
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    <input v-model="form.buyer_country" type="text" placeholder="India"
                           class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500" />
                </div>
            </div>
            <div class="grid grid-cols-2 gap-3">
