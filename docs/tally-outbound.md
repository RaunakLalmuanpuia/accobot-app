...
Returns pending company records for the tenant (queued when the user edits company details in Accobot). Every field received during inbound sync is echoed back — the full payload mirrors the inbound POST format.
...
Field notes:
- `TallyId` — Tally's assigned ID. `null` for new records; set for updates.
- `Guid` — the company GUID from Tally, used as the unique identifier.
- All `Is*` flags, deductor fields, and legacy VAT/ST/Excise/PF/ESI fields are stored in `feature_flags`, `deductor_details`, and `legacy_tax_details` jsonb columns respectively and spread back into the response as top-level keys.
